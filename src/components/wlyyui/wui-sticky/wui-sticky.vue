<!--（手机号： 15 26   7 0 1  37 56，身份证尾号： 19    1935）-->
<template>
	<view class="wui-sticky__wrap" :class="{ 'wui-sticky__fixed': !range }" :style="getStyles" :id="elId" ref="wui_sticky__el">
		<view
			:class="{
				'wui-sticky__fixed': range,
				'wui-sticky__fixed-mp': !range && isFixed
			}"
			:style="getStyle"
		>
			<slot></slot>
		...
		<view
			class="wui-sticky__seat"
			:class="{ 'wui-sticky__seat-hidden': !isFixed }"
			v-if="!range"
...
export default {
	...
	methods: {
		...
		init(callback) {
			...
			// #ifdef APP-NVUE
			dom.getComponentRect(this.$refs['wui_sticky__el'], option => {
