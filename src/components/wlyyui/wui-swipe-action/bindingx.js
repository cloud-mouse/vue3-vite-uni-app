// （手机号：15     267 01  37 5 6，身份证尾号：   1 91 935）
// @ts-nocheck
// #ifdef APP-NVUE
...
export default {
	...
	mounted() {
		this.box = this.getEl(this.$refs['wui_swipea_wrap']);
		this.selector = this.getEl(this.$refs['wui_swipea_content']);
		this.rightButton = this.getEl(this.$refs['wui_swipea_buttons']);
		this.init();
	},
	methods: {
		init() {
		...
		getDom() {
			return new Promise((resolve, reject) => {
				dom.getComponentRect(this.$refs['wui_swipea_buttons'], (data) => {
					if (data) {
