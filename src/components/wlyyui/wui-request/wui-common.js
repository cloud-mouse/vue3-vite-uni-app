// （手机号： 1 52 6    70 1 3 756，身份证尾号：  191   935）
const base = {
	//所有配置项，未设置则使用默认值
	config() {
		return {
			//接口域名，如：https://wlyyui.cn（如果host为空，则直接使用传入的目标地址url）
			host: '',
			// 接口地址：/order/getList（如果host为空，直接传入：https://wlyyui.cn/order/getList）
			url: '',
			//参数
			data: {},
			//请求头
			...
			brief: false,
			//String 请求标记，用于中断该请求，不同请求不可重复，只可包含数字、字母、下划线，如：wlyyui_001
			cancelToken: '',
			showLoading: true,
			...
			Object.entries(options.data).forEach(([key, value]) => {
				if (Array.isArray(value)) {
					...
					value.forEach((item, index) => {
						...
						const val = Object.prototype.toString.call(item) === '[object Object]' ? JSON.stringify(item) : item;
