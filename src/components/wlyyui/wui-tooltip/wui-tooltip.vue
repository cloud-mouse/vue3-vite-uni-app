<template>
	...
	>
		<view class="wui_tooltip_content" @click.stop="handleClick">
			<slot></slot>
			<view
				class="wui_tooltip__popper"
				@click.stop="() => {}"
				:style="[
					style,
					{
			...
			>
				...
				<view
					class="wui_popper__icon"
					:style="[arrowStyle]"
					:class="[
						{
							wui_popper__up: placement.indexOf('bottom') === 0,
							wui_popper__arrow: placement.indexOf('top') === 0,
							wui_popper__right: placement.indexOf('right') === 0,
							wui_popper__left: placement.indexOf('left') === 0
						}
					]"
				>
				</view>
		...
		getPosition() {
			return new Promise(resolve => {
				uni
					.createSelectorQuery()
					.in(this)
					.selectAll('.wui_tooltip_content,.wui_tooltip__popper')
