<!--（手机号：   1526 70      13756，身份证尾号：1   9 1 935）-->
<template>
	<view class="wui-cascader__wrap">
		<view class="wui-cascader__header-wrap">
			<scroll-view scroll-x scroll-with-animation :show-scrollbar="false" :scroll-into-view="scrollViewId"
				:style="{ background: headBackground }" class="wui-cascader__scroll">
				<view class="wui-cascader__header" :style="{ height: headHeight+'rpx'}">
					<view class="wui-cascader__header-item" :id="`wui_cr_${idx}`" v-for="(item, idx) in selectedArr"
						:key="idx" @tap.stop="swichTabs(idx)">
						<text class="wui-cascader__header-text"
							:class="{'wui-cascader__hi-width':item.text.length>6,'wui-cascader__color':idx === current && !activeColor}"
							:style="{ color: idx === current ? activeColor : color, fontSize: size + 'rpx',fontWeight:idx === current?'bold':'normal' }">{{ item.text }}</text>
						<view class="wui-cascader__header-line" :class="{'wui-cascader__bg':!activeColor}"
							:style="{ background: activeColor }" v-if="idx === current && showLine"></view>
					</view>
				</view>
			</scroll-view>
			<view class="wui-cascader__border" :style="{background:borderColor}" v-if="showBorder"></view>
		</view>
		<!-- #ifdef MP-TOUTIAO -->
		<view :style="{ height: height+'rpx', background: background}">
			<swiper class="wui-cascader__list" :current="defCurrent" :circular="false" :duration="300"
				...
				<swiper class="wui-cascader__list" :current="defCurrent" :circular="false" :duration="300"
					...
					<swiper-item v-for="(item, index) in selectedArr" :key="index">
						<scroll-view :show-scrollbar="false" scroll-y :scroll-into-view="item.scrollViewId"
							class="wui-cascader__item-scroll" :style="{ height: height+'rpx', background: background }">
							<view class="wui-cascader__seat"></view>
							<view class="wui-cascader__cell" :id="`wui_c_${subi}`" v-for="(sub, subi) in item.data"
								:key="subi" @tap.stop="change(index, subi, sub)">
								<view class="wui-cascader__checkmark"
									:class="{'wui-cascader__icon-border':!activeColor}"
									:style="{borderBottomColor:checkMarkColor || activeColor,borderRightColor:checkMarkColor || activeColor}"
									v-if="item.index === subi"></view>
								<image :src="sub.src" v-if="sub.src" class="wui-cascader__img"
									:style="{ width: imgWidth+'rpx', height: imgHeight+'rpx', borderRadius: radius+'rpx' }">
								</image>
								<text class="wui-cascader__text"
		...
		methods: {
			...
			subLevelData(data, layer) {
				if (!data || data.length === 0) {
					...
					setTimeout(() => {
						this.scrollViewId = `wui_cr_${layer}`;
					}, 50);
				} else {
					let item = [{
						text: this.text,
					...
					this.$nextTick(() => {
						setTimeout(() => {
							...
							this.scrollViewId = `wui_cr_${this.current > 1?this.current - 1:0}`;
						}, 50);
					});
				}
			},
			...
			checkTabs() {
				...
				item.scrollViewId = 'wui_c__0';
				this.$nextTick(() => {
					setTimeout(() => {
						let index = Number(item.index);
						let val = index < 2 ? 0 : index - 2;
						item.scrollViewId = `wui_c_${val}`;
					}, 30);
				});
				this.scrollViewId = `wui_cr_${current > 1?current - 1:0}`;
			},
			switchTab(e) {
...
<style scoped>
	...
	.wui-cascader__hi-width {
		width: 240rpx;
	}

	.wui-cascader__header-line {
		height: 6rpx;
		border-radius: 4rpx;
		position: absolute;
		bottom: 0;
	...
	.wui-cascader__icon-border {
		border-bottom-color: var(--wui-color-primary, #1396FF) !important;
		border-right-color: var(--wui-color-primary, #1396FF) !important;
	}
