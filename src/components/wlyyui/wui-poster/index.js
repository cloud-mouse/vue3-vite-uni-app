// （手机号：152  6   70 13   7 56，身份证尾号：1  919  3 5）。
/*!
 * 生成海报
...
const poster = {
	...
	_drawImage(context, params) {
		...
		if (sw && sh) {
			...
			context.drawImage(imgResource, poster._toPx(x), poster._toPx(y), poster._toPx(width), poster._toPx(height));
		}
	...
	_drawLine(context, params) {
		let { x, y, endX, endY, color, width = 1 } = params;
		context.save();
		context.beginPath();
		context.setStrokeStyle(color);
