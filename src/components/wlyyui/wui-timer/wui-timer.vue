<!--（手机号：15   2 670137 5     6，身份证尾号：1 9 1 93  5）-->
<template>
	<view class="wui-timer__wrap">
		<text
			class="wui-timer__item"
			:style="{
		...
		<view
			...
			v-if="isSeconds && isMs"
		>
			<view :class="{ 'wui-timer__ms-list': ani }" ref="wui_timer_ms">
				<text
					class="wui-timer__ms-item"
					:style="{
						height: height + 'rpx',
...
export default {
	...
	methods: {
		// #ifdef APP-NVUE
		_animation() {
			if (!this.$refs['wui_timer_ms'] || !this.isMs || !this.ani) return;
			animation.transition(
				this.$refs['wui_timer_ms'].ref,
				{
					styles: {
		...
		_aniEnd() {
			if (!this.$refs['wui_timer_ms'] || !this.isMs) return;
			animation.transition(this.$refs['wui_timer_ms'].ref, {
				styles: {
