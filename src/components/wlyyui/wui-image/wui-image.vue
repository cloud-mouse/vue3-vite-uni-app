<template>
	<view class="wui-image" @tap="onClick" :style="[wrapStyle, backgroundStyle]">
		<image
			v-if="!isError"
			:src="src"
			:mode="mode"
		...
		>
			...
			<wui-icon v-else :name="loadingIcon"></wui-icon>
		</view>
		<view
			v-if="showError && isError && !loading"
			class="wui-image__error"
			:style="{
				borderRadius: shape == 'circle' ? '50%' : $u.addUnit(borderRadius)
			}"
		>
			<slot v-if="$slots.error" name="error" />
			<wui-icon v-else :name="errorIcon"></wui-icon>
		</view>
	</view>
</template>
 ...
 * @property {Boolean} show-menwui-by-longpress 是否开启长按图片显示识别小程序码菜单，仅微信小程序有效（默认 false）
 ...
 * @example <wui-image width="100%" height="300rpx" :src="src"></wui-image>
 */
export default {
	name: 'wui-image',
	props: {
	...
	computed: {
		wrapStyle() {
			...
			style.borderRadius = this.shape == 'circle' ? '50%' : this.borderRadius + 'rpx';
