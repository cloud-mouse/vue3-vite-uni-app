<!--（手机号：1  52 67    01 3 75 6，身份证尾号：19  1 9  35）-->
<template>
	<view
		class="wui-bottom__popup-wrap"
		:class="{ 'wui-bottom__popwrap-show': show }"
	...
	>
		<!-- #ifdef APP-NVUE -->
		<view class="wui-bottom__popup-border" :style="{ height: radius + 'rpx', background: background }" v-if="radius"> </view>
		<!-- #endif -->
		<view
			ref="wui_bp_ani"
			class="wui-bottom__popup"
			:class="{
				'wui-bottom__popup-show': show,
				'wui-bp__safe-weex': iphoneX && safeArea
			}"
			:style="{
...
export default {
	...
	methods: {
		...
		_animation(type) {
			if (!this.$refs['wui_bp_ani'] || !this.$refs['wui_bp_mk_ani']) return;
			animation.transition(
				this.$refs['wui_bp_mk_ani'].ref,
				{
					styles: {
						opacity: type ? 1 : 0
					},
