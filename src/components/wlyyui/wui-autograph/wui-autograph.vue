<template>
	<view class="wui-autograph__wrap" :style="{ width: w + 'px', height: h + 'px', background: background }" ref="wui_gcanvas_ag">
		<!-- #ifdef APP-NVUE -->
		<gcanvas
...
// #endif
// #ifdef MP-WEIXIN
const canvasId = `wui_ag_${Math.ceil(Math.random() * 10e5).toString(36)}`;
// #endif
export default {
	name: 'wui-autograph',
	emits: ['ready'],
	...
	data() {
		...
		canvasId = `wui_ag_${Math.ceil(Math.random() * 10e5).toString(36)}`;
		// #endif
		// #ifdef MP-QQ
		canvasId = 'canvas_autograph';
		// #endif
		return {
			// #ifdef APP-NVUE
			isAndroid: true,
	...
	methods: {
		// #ifdef APP-NVUE
		getDom() {
			dom.getComponentRect(this.$refs['wui_gcanvas_ag'], option => {
