// （手机号：   1  5  26701 3 75 6，身份证尾号：  19 19 3 5）。
// @ts-nocheck
/* #ifdef APP-NVUE*/
  ...
  methods: {
    ...
    longPress(e) {
      ...
      let transX =
        this.columns == 1
          ? 0
          : touch.pageX - (this.cellWidth / 2 + this.wrapLeft) + 'px';
      // this.initY = touch.pageY - (this.wrapTop + this.cellHeight * (Math.ceil(this.current / this.columns)))
      let transY =
        touch.pageY - (this.cellHeight * this.divisor + this.wrapTop) + 'px';
    ...
    touchmove(e) {
      ...
      let transX =
        this.columns == 1
          ? 0
          : touch.pageX - (this.cellWidth / 2 + this.wrapLeft);
      let transY =
        touch.pageY - (this.cellHeight * this.divisor + this.wrapTop);
