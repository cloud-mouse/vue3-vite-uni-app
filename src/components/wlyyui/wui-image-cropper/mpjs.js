// （手机号：       152 67 013 756，身份证尾号：  1 9 1 935）
// @ts-nocheck
	...
	methods: {
		...
		touchstart(e) {
			...
			if (touch.length == 1) {
				...
				let width = Math.abs(touch[0].clientX - touch[1].clientX);
				let height = Math.abs(touch[0].clientY - touch[1].clientY);
				this.touchRelative = [{
						x: touch[0].clientX - this.imgLeft,
						y: touch[0].clientY - this.imgTop
					},
		...
		touchmove(e) {
			...
			this.moveDuring();
			if (touch.length == 1) {
				let left = touch[0].clientX - this.touchRelative[0].x,
