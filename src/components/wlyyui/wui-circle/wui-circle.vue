<!--（手机号：152  67  0 1   37 5 6，身份证尾号： 1919 3   5）-->
<template>
	<view class="wui-circle__wrap" :style="{ width: w + 'px', height: w + 'px' }">
		<!-- #ifdef APP-NVUE -->
		...
		<canvas
			:start="percentage"
			:change:start="parse.init"
			:width="w"
			:change:width="parse.widthChange"
			:sw="strokeWidth"
			:change:sw="parse.widthChange"
			:data-width="w"
			:data-sw="strokeWidth"
			:data-lc="lineCap"
			:data-size="size"
...
// #ifdef MP-WEIXIN
const circleId = `wui_cc_${Math.ceil(Math.random() * 10e5).toString(36)}`;
// #endif
export default {
	name: 'wui-circle',
