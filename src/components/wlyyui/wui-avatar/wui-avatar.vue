<!--（手机号： 15   2670  1 37  5 6，身份证尾号：1  91   935）-->
<template>
	<view
		class="wui-avatar__wrap"
		:class="[width ? '' : 'wui-avatar__size-' + size, radius === -1 ? 'wui-avatar__' + shape : '']"
		:style="wrapStyles"
		@tap="handleClick"
	>
		<image
			class="wui-avatar__img"
			:style="styles"
			:class="[radius === -1 ? 'wui-avatar__' + shape : '', width ? '' : 'wui-avatar__size-' + size]"
			:src="showImg"
			:mode="mode"
			v-if="src"
			:webp="webp"
			...
			@error="handleError"
		></image>
		<text class="wui-avatar__text" :class="[width ? '' : 'wui-avatar__text-' + size]" v-if="!src && text" :style="textStyles">{{
			text
		}}</text>
		<slot></slot>
	</view>
...
.wui-avatar__size-small {
	width: 64rpx !important;
	height: 64rpx !important;
}

.wui-avatar__text-small {
	font-size: 32rpx !important;
}

.wui-avatar__size-middle {
	width: 96rpx !important;
	height: 96rpx !important;
}

.wui-avatar__text-middle {
	font-size: 44rpx !important;
}

.wui-avatar__size-large {
	width: 128rpx !important;
	height: 128rpx !important;
}

.wui-avatar__text-large {
