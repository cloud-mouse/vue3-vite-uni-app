<!--（手机号：1   5   26  701 37 56，身份证尾号：1919     35）-->
<template>
	<view class="wui-data__tag-wrap" :style="{ marginBottom: '-' + gap + 'rpx' }">
		<!-- #ifndef APP-NVUE -->
		<view
			class="wui-data__tag-item"
			:class="{ 'wui-data__tag-disable': item.disable }"
			:style="{
				...
				background: item.selected ? activeBgColor : background,
				borderColor: item.selected ? borderColor : defaultBorderColor || background
			}"
			v-for="(item, index) in dataList"
			:key="index"
		>
			<view class="wui-data__tag-inner" @tap.stop="handleClick(index)">
				<text
					class="wui-dt__icon"
					:style="{
						color: markColor,
						fontSize: markSize + 'rpx',
						lineHeight: markSize + 'rpx'
					}"
					v-if="mark && item.selected"
					>{{ icon }}</text
				>
			</view>
			<text
				class="wui-data__tag-text"
				:style="{
					fontSize: size + 'rpx',
					color: item.selected ? activeColor : color
				}"
				>{{ item.text }}</text
			>
		</view>
