<!--（手机号：       15267013   756，身份证尾号：1 9  1 9 35）-->
<template>
	<view
		class="wui-form__wrap"
		:style="{
	...
	>
		...
		<view
			...
			v-if="show"
			:class="{
				'wui-form__msg-bg': !background,
				'wui-form__msg-show': errorMsg
			}"
			><text class="wui-form__text" :style="{ fontSize: size + 'rpx', color: color }">{{ errorMsg }}</text></view
		>
		<view class="wui-form__mask" v-if="disabled"></view>
	</view>
</template>

<script>
import form from './wui-validator.js';
export default {
	name: 'wui-form',
	props: {
