<!--（手机号：   1  5 2   67013 756，身份证尾号：1 9 193   5）-->
<template>
	<gcanvas class="wui-poster__canvas" :ref="canvasId" :style="{ width: w + 'px', height: h + 'px' }"> </gcanvas>
</template>

<script>
/*!
 * 生成海报
 * nvue端专用，需真机运行查看效果，其他端请使用wui-poster组件
 * poster - v1.0.0 (2021/7/19, 16:52:14 PM)
 *
 */
import { enable, WeexBridge } from './gcanvas/index.js';
export default {
	name: 'wui-poster-weex',
	emits: ['ready'],
	props: {
		//画布宽度，单位rpx
		width: {
	...
	methods: {
		...
		_drawRadiusRect(context, params) {
			let { x, y, width, height, borderRadius } = params;
		...
		_drawImage(context, params) {
			let { imgResource, x, y, width, height, sx, sy, sw, sh, borderRadius = 0, borderWidth = 0, borderColor } = params;
			context.save();
			if (borderRadius > 0) {
			...
			if (sw && sh) {
				...
				context.drawImage(imgResource, this._toPx(x), this._toPx(y), this._toPx(width), this._toPx(height));
			}
		...
		_drawLine(context, params) {
			let { x, y, endX, endY, color, width } = params;
			context.save();
			context.beginPath();
			context.setStrokeStyle(color);
