<!--（手机号：1 526  7  0 1    3756，身份证尾号： 19193    5）-->
<template>
	<view :class="{ 'wui-loading__mask': (isMask || isNvue) && isFixed }">
		<view
			class="wui-loading wui-loading__wrap"
			:class="{ 'wui-loading__fixed': isFixed && !isNvue }"
			:style="{
				backgroundColor: backgroundColor,
				position: isFixed && !isNvue ? 'fixed' : 'static'
			}"
			v-if="type === 'col'"
		>
			<image ref="wui_loading" class="wui-loading__ani" :src="srcCol"></image>
			<text
				class="wui-loading__text"
				:style="{
					color: colColor,
					fontSize: size + 'rpx',
		...
		<view
			v-else
			class="wui-loading wui-loading__row"
			:class="{ 'wui-loading__fixed': isFixed && !isNvue }"
			:style="{ position: isFixed && !isNvue ? 'fixed' : 'static' }"
		>
			<!-- <loading-indicator :animating="true"></loading-indicator> -->
			<image ref="wui_loading" class="wui-loading-row__ani" :src="srcRow"></image>
			<text
				class="wui-loading__text"
				:style="{
					color: rowColor,
...
export default {
	...
	methods: {
		// #ifdef APP-NVUE
		_animation() {
			if (!this.$refs['wui_loading'] || this.stop) return;
			animation.transition(
				this.$refs['wui_loading'].ref,
				{
					styles: {
