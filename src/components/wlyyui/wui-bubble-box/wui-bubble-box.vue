<!--（手机号：1    5 267   0137  56，身份证尾号：19 19 3   5）-->
<template>
	<view class="wui-bubble__box-wrap" ref="wui_bb_wrap">
		<slot></slot>
		<view
			@touchmove.stop.prevent="stop"
			class="wui-bubble__box-mask"
			:class="[show ? 'wui-bb__unnvue-show' : '', isShow ? 'wui-bb__opciaty-show' : 'wui-bb__opciaty-hidden']"
			:style="getMaskStyl"
			v-if="isMask && isVisible"
			@tap="handleClose"
		></view>
		<view
			class="wui-bubble__box-inner"
			ref="wui_bb_el"
			:style="getBbStyl"
			:class="[
				direction === 'top' || direction === 'bottom' ? 'wui-bubble__box-col' : 'wui-bubble__box-row',
				position === 'absolute' ? 'wui-bb__list-' + direction : '',
				show ? 'wui-bb__unnvue-show' : '',
				isShow ? 'wui-bb__opciaty-show' : 'wui-bb__opciaty-hidden'
			]"
			v-if="isVisible"
		>
			<view class="wui-bb__triangle-wrap" :style="getStyle" v-if="direction === 'bottom' || direction === 'right'">
				<view class="wui-bubble__box-triangle" :style="getStyles" :class="['wui-bb__triangle-' + direction]"> </view>
			</view>
			<view class="wui-bubble__box-list" :style="{ background: background, width: width + 'rpx' }">
				<view
					class="wui-bubble__box-item"
					:style="{
				...
				>
					<wui-icon :name="item.name" :size="item.size || 60" :color="item.color || color" v-if="item.name"> </wui-icon>
					<image
						:src="item.src"
						:style="{
						...
						v-if="item.src && !item.name"
					></image>
					<text
						:class="{ 'wui-bubble__box-text': item.name || item.src }"
						:style="{
							fontSize: size + 'rpx',
							color: color,
					...
					>
					<view
						class="wui-bubble__box-line"
						:class="[direction === 'left' ? 'wui-bb__line-right' : 'wui-bb__line-left']"
						:style="{
							background: lineColor,
							left: direction === 'left' ? 0 : padding[3] || padding[1] || 0,
							right: direction === 'left' ? padding[3] || padding[1] || 0 : 0
						}"
						v-if="index !== items.length - 1 && showLine"
					></view>
				</view>
				<slot name="content"></slot>
			</view>
			<view class="wui-bb__triangle-wrap" :style="getStyle" v-if="direction === 'top' || direction === 'left'">
				<view class="wui-bubble__box-triangle" :style="getStyles" :class="['wui-bb__triangle-' + direction]"> </view>
			</view>
		</view>
	</view>
...
export default {
	...
	methods: {
		...
		init(callback) {
			...
			dom.getComponentRect(this.$refs['wui_bb_wrap'], option => {
		...
		_animation(type) {
			if (this.position === 'fixed') {
				...
				if (!this.$refs['wui_bb_el']) return;
				animation.transition(
					this.$refs['wui_bb_el'].ref,
					{
						styles: {
...
<style scoped>
.wui-bubble__box-wrap {
	position: relative;
}

.wui-bubble__box-inner {
	/* #ifndef APP-NVUE */
	display: flex;
...
.wui-bubble__box-item:active {
	background: rgba(0, 0, 0, 0.2);
}

.wui-bubble__box-line {
	position: absolute;
	bottom: 0;
	/* #ifdef APP-NVUE */
	height: 0.5px;
...
.wui-bubble__box-text {
	padding-left: 16rpx;
}

.wui-bubble__box-mask {
	position: fixed;
	top: 0;
	left: 0;
	right: 0;
...
.wui-bb__triangle-wrap {
	position: relative;
	overflow: hidden;
}

.wui-bubble__box-triangle {
	width: 10px;
	height: 10px;
	position: absolute;
}

.wui-bb__triangle-top {
	/* #ifndef APP-NVUE */
...
/* #ifdef APP-NVUE */
.wui-bb__opciaty-show {
	opacity: 1;
}

.wui-bb__opciaty-hidden {
	opacity: 0;
}
