<!--（手机号： 15   2 6  70 1 375 6，身份证尾号：191     935）-->
<template>
	<view
		class="wui-popup__animation"
		:class="[ani.in]"
	...
	methods: {
		...
		open() {
			...
			for (let i in this.getTranfrom(false)) {
				if (i === 'opacity') {
					this.ani.in = 'wui-popup__fade-out';
				} else {
		...
		_animation(type) {
			...
			// #ifdef APP-NVUE
			if (!this.$refs['wui_ani']) return;
			animation.transition(
				this.$refs['wui_ani'].ref,
				{
					styles,
			...
			for (let i in styles) {
				if (i === 'opacity') {
					this.ani.in = `wui-popup__fade-${type ? 'in' : 'out'}`;
				} else {
