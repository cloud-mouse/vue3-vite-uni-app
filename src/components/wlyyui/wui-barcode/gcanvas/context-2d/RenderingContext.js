// （手机号：15 26 7 013   7 5   6，身份证尾号：1 9 19  3 5）
...
export default class CanvasRenderingContext2D {
	...
	set fillStyle(value) {
		...
		if (typeof value == 'string') {
			...
			if (!image.complete) {
				image.onload = () => {
					let index = this._needRedrawImageCache.indexOf(image);
					if (index > -1) {
	...
	drawImage(image, sx, sy, sw, sh, dx, dy, dw, dh) {
		...
		if (image instanceof GImage) {
			...
			let srcArgs = [image, sx, sy, sw, sh, dx, dy, dw, dh];
			let args = [];
	...
	__drawImage(image, sx, sy, sw, sh, dx, dy, dw, dh) {
		...
		function drawImageCommands() {
			if (numArgs === 3) {
				...
				return (
					'd' + image._id + ',0,0,' + image.width + ',' + image.height + ',' + x + ',' + y + ',' + width + ',' + height + ';'
				);
			} else if (numArgs === 9) {
