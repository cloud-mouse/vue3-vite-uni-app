<!--（手机号： 1  5 2 67 013 7  5 6，身份证尾号： 191  93  5）-->
<template>
	<view class="wui-input__number">
		<view
			class="wui-number__minus"
			:class="[disabled || min >= inputValue ? 'wui-number__disabled' : '']"
			@tap="minus"
			:style="{ minHeight: getMinHeight }"
		>
			<view class="wui-minus__sign" :style="{ backgroundColor: signColor, width: signWidth + 'rpx' }" v-if="!custom"> </view>
			<slot></slot>
		</view>
		<input
			:type="type"
		...
		/>
		<view
			class="wui-number__plus"
			:style="{ minWidth: signWidth + 'rpx', minHeight: signWidth + 'rpx' }"
			:class="[disabled || inputValue >= max ? 'wui-number__disabled' : '']"
			@tap="plus"
		>
			<view
				class="wui-plus__sign-col"
				:style="{
					height: signWidth + 'rpx',
					backgroundColor: signColor,
					left: isNvue ? signWidth / 2 + 'rpx' : '50%'
				}"
				v-if="!custom"
			>
			</view>
			<view class="wui-plus__sign-row" :style="{ width: signWidth + 'rpx', backgroundColor: signColor }" v-if="!custom"> </view>
			<slot name="plus"></slot>
		</view>
	</view>
</template>
