 ...
 */
export function chooseVideo(type: any = 'album', ossData: any, filePathName: string, callBack: any) {
	const ossObj = {
		...
		uni.chooseMedia({
			...
			success: function (res: any) {
				if (res.errMsg == 'chooseMedia:ok') {
					...
					const option = {
						savePath: filePathName || 'app/video',
						...ossObj
					};
					if (ossData.provider === 'Tencent') {
						...
						AliOss.upload(filePath, option, function (resVideo: any) {
							if (resVideo) {
								callBack(resVideo);
							} else {
								Toast('视频上传失败，请稍后重试');
	...
	if (type === 'camera') {
		uni.chooseMedia({
			...
			camera: 'back',
			success(res: any) {
				const filePath = res && res.tempFiles && res.tempFiles[0].tempFilePath;
				const option = {
